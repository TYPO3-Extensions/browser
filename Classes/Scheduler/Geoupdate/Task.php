 ...
 *
 *
 *  119: class tx_browser_Geoupdate extends AbstractTask
 *
 *              SECTION: Main
...
class Task extends \TYPO3\CMS\Scheduler\Task\AbstractTask
{
  ...
  public function execute()
  {
    ...
    {
      $prompt = 'Success!';
      GeneralUtility::devLog( '[tx_browser_Geoupdate]: ' . $prompt, $this->extKey, -1 );
    }
    // DRS
    // E-mail to admin
  ...
  private function geoupdateInitRows()
  {
    ...
    $labels = array( 'uid' ) + $this->geoupdatelabels[ 'address' ] + $this->geoupdatelabels[ 'api' ] + $this->geoupdatelabels[ 'geodata' ]
    ;
...
//    $this->log( $prompt, -1 );
  ...
  private function geoupdateRequired()
  {
    ...
    switch ( true )
    {
      case(!$update ):
        ...
        if ( $this->drsModeInfo )
        {
          GeneralUtility::devLog( '[tx_browser_Geoupdate]: ' . $prompt, $this->extKey, 2 );
        }
        // E-mail to admin
        $subject = 'Disabled';
        $body = $prompt . PHP_EOL
      ...
      case( empty( $geodata ) ):
        ...
        if ( $this->drsModeInfo )
        {
          GeneralUtility::devLog( '[tx_browser_Geoupdate]: ' . $prompt, $this->extKey, 3 );
        }
        // E-mail to admin
        $subject = 'Failed!';
        $body = $prompt . PHP_EOL
  ...
  {
    $this->geoupdateStatisticData[ 'rows' ] = $this->geoupdateStatisticData[ 'addressEmpty' ] + $this->geoupdateStatisticData[ 'forbidden' ] + $this->geoupdateStatisticData[ 'geodataNotEmpty' ] + $this->geoupdateStatisticData[ 'errors' ] + $this->geoupdateStatisticData[ 'updatedTest' ] + $this->geoupdateStatisticData[ 'updated' ]
    ;
...
';
  ...
  private function geoupdateUpdateRowRequiredAddress( $row )
  {
    ...
    $this->geoupdateStatisticData[ 'addressEmpty' ] = $this->geoupdateStatisticData[ 'addressEmpty' ] + 1
    ;
  ...
  private function geoupdateUpdateRowRequiredGeodata( $row )
  {
    ...
    foreach ( $this->geoupdatelabels[ 'geodata' ] as $label )
    {
      ...
      $this->log( $prompt, -1, $row[ 'uid' ] );
      // Statistic
      $this->geoupdateStatisticData[ 'geodataNotEmpty' ] = $this->geoupdateStatisticData[ 'geodataNotEmpty' ] + 1
      ;
      return false;
    }
    // RETURN : false, latitude or longitude contain content at least
  ...
  private function geoupdateUpdateRowRequiredPermission( $row )
  {
    ...
    if ( $row[ $this->geoupdatelabels[ 'api' ][ 'forbidden' ] ] )
    {
      ...
      $this->geoupdateStatisticData[ 'forbidden' ] = $this->geoupdateStatisticData[ 'forbidden' ] + 1
      ;
      return false;
    }
  ...
  private function geoupdateUpdateRowUpdateDataSet( $row )
  {
    ...
    if ( isset( $result[ 'status' ] ) )
    {
      ...
      // Statistic
      $this->geoupdateStatisticData[ 'errors' ] = $this->geoupdateStatisticData[ 'errors' ] + 1
      ;
      return false;
    }
    // Prompt to current record
  ...
  private function geoupdateUpdateRowUpdateDataUpdate( $row )
  {
    ...
    $browser = ' - ' . $GLOBALS[ 'LANG' ]->sL( 'LLL:EXT:browser/Resources/Private/Language/Scheduler/locallang.xml:promptBrowserPhrase' ) . ':';
    $updatePrompt = '* ' . $date . $browser . PHP_EOL
            . '  OK: ' . $GLOBALS[ 'LANG' ]->sL( 'LLL:EXT:browser/Resources/Private/Language/Scheduler/locallang.xml:promptGeodataUpdate' )
            . $prompt . PHP_EOL
    ...
    {
      case( 'enabled' ):
        $this->geoupdateStatisticData[ 'updatedTest' ] = $this->geoupdateStatisticData[ 'updatedTest' ] + 1
        ;
    ...
    // Statistic
    $this->geoupdateStatisticData[ 'updated' ] = $this->geoupdateStatisticData[ 'updated' ] + 1
    ;
    return true;
  }
...
table     : ' . $this->browser_table;
  ...
  private function initDRS()
  {
    ...
    GeneralUtility::devlog( '[tx_browser_Geoupdate] ' . $prompt, $this->extKey, 0 );
...
//    return $os;
   ...
   * @param	mixed     $uid    : uid of the current record
   * @return	void
   * @access private
   * @version   7.4.5
   * @since     4.5.7
   */
  private function log( $prompt, $status = 0, $uid = 0 )
  {
    $table = $this->browser_table;
    ...
    switch ( $status )
    {
      ...
      case( 0 ):
        ...
        $fmStatus = FlashMessage::NOTICE;
        $logStatus = 0;
        break;
      case( 1 ):
        $fmHeader = 'Geocoding by Browser - TYPO3 without PHP';
        $fmStatus = FlashMessage::INFO;
        $logStatus = 0;
        break;
      case( 2 ):
        $fmHeader = 'Geocoding by Browser - TYPO3 without PHP';
        $fmStatus = FlashMessage::OK;
        $logStatus = 0;
        break;
      case( 3 ):
        $fmHeader = 'Geocoding by Browser - TYPO3 without PHP';
    ...
    $GLOBALS[ 'BE_USER' ]->simplelog( $details, $this->extKey, $logStatus );
    if ( $logStatus > 0 )
    {
    ...
    $message = GeneralUtility::makeInstance(
                    'TYPO3\\CMS\\Core\\Messaging\\FlashMessage', $fmPrompt, $fmHeader, // the header is optional
                    $fmStatus, // the severity is optional as well and defaults to \TYPO3\CMS\Core\Messaging\FlashMessage::OK
...
//    $flashMessage = GeneralUtility::makeInstance( 'FlashMessage', $fmPrompt, $fmHeader, $fmStatus );
...
//    require_once( PATH_t3lib . 'class.t3lib_timetrack.php' );
    ...
    {
      $this->TT = new \TYPO3\CMS\Core\TimeTracker\TimeTracker();
    }
  ...
  private function timeTracking_log( $debugTrailLevel, $prompt )
  {
    ...
    GeneralUtility::devLog( $prompt, $this->extKey, 0 );
    ...
    switch ( true )
    {
      case( $timeOfPrevProcess >= 10000 ):
        ...
        GeneralUtility::devLog( '[WARN/PERFORMANCE] ' . $prompt, $this->extKey, 3 );
        break;
      case( $timeOfPrevProcess >= 250 ):
        ...
        GeneralUtility::devLog( '[WARN/PERFORMANCE] ' . $prompt, $this->extKey, 2 );
        break;
      default:
  ...
  private function timeTracking_prompt( $debugTrailLevel, $prompt )
  {
    ...
    GeneralUtility::devLog( '[INFO/PERFORMANCE] ' . $prompt, $this->extKey, $this->tt_prevPrompt );
  }
