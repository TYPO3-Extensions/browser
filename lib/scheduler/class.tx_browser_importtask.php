 ...
 *
 *
 *  105: class tx_browser_ImportTask extends tx_scheduler_Task
 *
 *              SECTION: Main
...
/**
 * Class "tx_browser_ImportTask" provides procedures for check import and control browser mailboxes
 *
 ...
 * @since         0.0.1
 */
class tx_browser_ImportTask extends tx_scheduler_Task {
  ...
  public function execute( )
  {
    ...
    if( ! $xml )
    {
      ...
      if( $this->drsModeError )
      {
        ...
        t3lib_div::devLog( '[tx_browser_ImportTask]: ' . $prompt, $this->extKey, 3 );
      }
        // DRS
      $this->timeTracking_log( $debugTrailLevel, 'END' );
      return false;
    ...
    if( $this->get->getContentIsUpToDate( ) )
    {
      ...
      if( $this->drsModeInfo )
      {
        ...
        t3lib_div::devLog( '[tx_browser_ImportTask]: ' . $prompt, $this->extKey, -1 );
      }
        // DRS
    ...
    switch( true )
    {
      case( $this->browser_reportMode == 'never' ):
        ...
        if( $this->drsModeInfo )
        {
          ...
          t3lib_div::devLog( '[tx_browser_ImportTask]: ' . $prompt, $this->extKey, 2 );
        }
          // DRS
        return;
        break;
 ...
 */
  private function initDRS( )
  {
    ...
    t3lib_div::devlog( '[tx_browser_ImportTask] ' . $prompt, $this->extKey, 0 );
...
if (defined('TYPO3_MODE') && isset($GLOBALS['TYPO3_CONF_VARS'][TYPO3_MODE]['XCLASS']['ext/browser/lib/scheduler/class.tx_browser_importtask.php'])) {
  include_once($GLOBALS['TYPO3_CONF_VARS'][TYPO3_MODE]['XCLASS']['ext/browser/lib/scheduler/class.tx_browser_importtask.php']);
}
