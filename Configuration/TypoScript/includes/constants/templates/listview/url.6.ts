plugin.tx_browser_pi1 {
  ...
  # cat=Browser - Templates - List - URL 6*//106;        type=options[,target,url];  label= return:empty: a-tag (recommended). url: returns the URL only. Recommended for a rss feed. target: return the target only.
  templates.listview.url.6.returnLast =
  # cat=Browser - Templates - List - URL 6*//107;        type=string;  label= Alias for showUid:If the destination tables is using an alias for the showUid, you must enter it. I.e: jobUid
  templates.listview.url.6.showUid  = showUid
  # cat=Browser - Templates - List - URL 6*//108;        type=int+;    label= PID of single view:Page id of the single view. Leave it empty, if the list view and single have the same pid. I.e: 142
  templates.listview.url.6.singlePid  =
  # cat=Browser - Templates - List - URL 6*//999; type=user[EXT:browser/lib/class.tx_browser_extmanager.php:tx_browser_extmanager->promptExternalLinks]; label=Helpful links
