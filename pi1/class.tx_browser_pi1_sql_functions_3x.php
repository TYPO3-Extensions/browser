...
//$this->pObj->dev_var_dump( $conf_view['aliases.']['tables.'] );
  ...
  function get_propper_andWhere( $str_andWhere )
  {
    ...
    foreach ( ( array ) $arr_andWhere_withMaskedStrings as $key => $value )
    {
      switch ( $key % 2 )
      {
        case(true):
          ...
          if ( !$boolMaskOdd )
            $arr_andWhere_withMaskedStrings[ $key ] = 'dummy';
          break;
        case(false):
    ...
    if ( $this->pObj->b_drs_sql && $str_andWhere )
      t3lib_div::devlog( '[INFO/SQL] andWhere clause is: ' . $str_andWhere, $this->pObj->extKey, 0 );
...
//    $lConfCObj        = $this->pObj->objZz->substitute_t3globals_recurs($lConfCObj);
    // #29198, 110824, dwildt-
    // #29198, 110824, dwildt-
//    $str_tsValue      = $this->pObj->objWrapper4x->general_stdWrap($this->pObj->local_cObj->COBJ_ARRAY($lConfCObj, $ext=''), false);
    // #29198, 110824, dwildt+
