 ...
 *  Copyright notice
 *
 *  (c) 2012-2015 - Dirk Wildt <http://wildt.at.die-netzmacher.de>
 *  All rights reserved
 *
...
class tx_browser_pi1_sql_functions
{
  ...
  /**
   * getStatementWoAsAndAlias( ) : Get the SQL part behind the AS. If this is an alias, replace the alias with the real name.
   *
   * @param	array         $statement: Array with table.field values maybe with an AS
   * @return	array         $statement with real names of table.fields
   *
   * @version   5.0.0
   * @since     5.0.0
   */
  private function getStatementWoAsAndAlias( $statement )
  {
    ...
    // LOOP each part of the statemant
    foreach ( ( array ) $parts as $key => $part )
    {
      $part = $this->expressionToAlias( $part );
      $part = $this->getAlias($part );

      // CONTINUE : part is empty
      if ( empty( $part ) )
      {
        unset( $parts[ $key ] );
        continue;
    ...
    $parts = $this->aliasToTable( $parts );
    $statement = implode( ',', $parts );
    return $statement;
  }

//  /**
//   * getStatementHandleDealAsTable( ) :
//   *
//   * @param	string         $part : part of the current statement
//   * @return	string       $part : part of the current statement
//   *
//   * @version   5.0.0
//   * @since     5.0.0
//   */
//  private function handleDealAsTable( $part )
//  {
//    $conf_view = $this->conf;
//
...
//    }
//
//    // LOOP deal as table properties
//    foreach ( $conf_view[ 'select.' ][ 'deal_as_table.' ] as $properties )
//    {
//      $arrCsv = $properties[ 'statement' ];
//      $alias = $properties[ 'alias' ];
//      $newPart = str_replace( $arrCsv, $alias, $part );
//
//      // CONTINUE : property hasn't any effect
//      if ( $newPart == $part )
//      {
//        continue;
//      }
...
//
//      // DRS prompt
//      $prompt = 'Statement is replaced with alias. '
//              . 'Statement: "' . $arrCsv . '. '
//              . 'Alias: "' . $alias . '"'
//      ;
//      t3lib_div::devlog( '[INFO/SQL] ' . $prompt, $this->pObj->extKey, 0 );
...
//  }
  ...
  /**
   * getStatementWoAscDesc( ) : Returns table.field or the alias.
   ...
   * @since   5.0.0
   */
  public function getStatementWoAscDesc( $statement )
  {
    $statement = $this->getStatementWoAsAndAlias( $statement );

    // Clean up line feeds, carriage returns, double spaces
    $statement = $this->pObj->objZz->cleanUp_lfCr_doubleSpace( $statement );

    // Remove ASC, DESC and spaces
    $statement = str_ireplace( ' desc', null, $statement );
    $statement = str_ireplace( ' asc', null, $statement );
  ...
  public function cObjGetSingle( $currConfPath, $statement, $coa_name, $coa_conf )
  {
    ...
    if ( empty( $statement ) )
    {
      if ( $this->pObj->b_drs_sql )
      {
        $prompt = 'Statement / COA type is empty.';
        t3lib_div::devlog( '[INFO/SQL] ' . $prompt, $this->pObj->extKey, 0 );
        $prompt = 'Maybe empty statement / empty COA type is proper in case of a manual configured SQL query.';
        t3lib_div::devLog( '[INFO/SQL] ' . $prompt, $this->pObj->extKey, 1 );
