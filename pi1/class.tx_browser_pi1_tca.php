...
// Variables set by the pObj (by class.tx_browser_pi1.php)
   ...
   * @return	void
   */
  public function setArrHandleAs()
  {
...
// Get the selected fields out of the TS
   ...
   * @since   2.0.0
   */
  private function autodiscConfig()
  {
    ...
    // 140620, dwildt, 1+
    switch ( is_array( $this->pObj->confAutodiscover ) )
    {
      case(true):
...
//
   ...
   */
  private function autodiscTCA( $tableField )
  {
...
// Explode table name and field name from the value $tableField;
    ...
    foreach ( ( array ) array_keys( ( array ) $TSconfAdItems ) as $itemKey )
    {
...
// Add the table.field to the list of comma seperated values.
        ...
        if ( $uploadfolder )
        {
          $this->pObj->arrHandleAs[ 'uploadfolder' ][ $TCAitem ] = $uploadfolder;
        }
        if ( $this->pObj->b_drs_discover )
        {
...
// Dyeing swords?
    ...
    $this->value = $this->pObj->objWrapper4x->wrapDocument( $this->value );
...
// tableField has a content_stdWrap
    ...
    {
      $this->value = $this->pObj->objWrapper4x->general_stdWrap(
              $this->value, $this->lDisplayView[ 'content_stdWrap.' ]
      );
    }
// tableField has a content_stdWrap
// DRS - Development Reporting System
...
// Dyeing swords?
    ...
    $this->value = $this->pObj->objWrapper4x->wrapYYYYMMDD( $this->value );
