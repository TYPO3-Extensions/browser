...
//  var $bool_dontLocalise      = null;
  ...
  public $andWhereFilter = null;
  // [Array] andWhere statement as array
  public $arr_andWhereFilter = null;
...
//$this->pObj->dev_var_dump( $this->pObj->objCal->getVarRequirements( ) );
  ...
  /**
   * init_andWhereFilter_localTableAreaWiCal( ) : In case of Browser +Calendar
   *
   * @param	array		$arr_piVar   Current piVars
   * @param	string		$tableField   Current table.field
   * @return	array		arr_andWhereFilter: NULL if there isn' any filter
...
//        $arr_orValues[] = '(' . implode(' AND ', $arr_item) . ') ';
  ...
  /**
   * init_andWhereFilter_localTableAreaWoCal( ) : In case of no Browser +Calendar
   *
   * @param	array		$arr_piVar   Current piVars
   * @param	string		$tableField   Current table.field
   * @return	array		arr_andWhereFilter: NULL if there isn' any filter
...
//    }
  ...
  /**
   * get_filterItemForeigntable( ): Alias for get_filterItem
   *
   * @param	integer		$uid        : uid of the current item / row
   * @param	string		$value      : value of the current item / row
   * @return	string		$item       : The rendered item
  ...
  /**
   * get_filterItemLocaltable( ): Alias for get_filterItem
   *
   * @param	integer		$uid        : uid of the current item / row
   * @param	string		$value      : value of the current item / row
   * @return	string		$item       : The rendered item
  ...
  private function get_filterItemLocaltable( $uid, $value, $items )
  {
    ...
    //  * the following workflow can't order this values after exploding
    //  * the query for filter data would need a new concept
//    $values = explode( ',', $csvValue );
//
//    foreach( $values as $value )
//    {
//      $currRowValue = $this->rows[$uid][ $this->curr_tableField ];
//      $this->rows[$uid][ $this->curr_tableField ] = $value;
//      $item         = $this->get_filterItem( $uid, $value );
//      $this->rows[$uid][ $this->curr_tableField ] = $currRowValue;
//      $items        = $items . $this->htmlSpaceLeft . ' ' . $item . PHP_EOL ;
//    }
//
//    return $items;
