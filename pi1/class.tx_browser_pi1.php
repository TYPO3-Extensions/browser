 ...
 *
 *
 *  106: class tx_browser_pi1 extends tslib_pibase
 *
 *              SECTION: Main Process
 *  424:     public function main( $content, $conf )
 *
 *              SECTION: DRS - Development Reporting System
 * 1308:     public function drs_debugTrail( $level = 1 )
 * 1357:     public function dev_var_dump( $content )
  //public function dev_var_dump( )
 *
 *              SECTION: Classes
 * 1430:     private function require_classes()
 *
 *              SECTION: Init
 * 1612:     private function init( $conf )
 * 1659:     private function init_accessByIP( )
 * 1699:     private function init_classVars( )
 * 2047:     public function init_drs()
 * 2385:     private function init_typoscriptVersion( )
 * 2421:     private function init_typo3version( )
 *
 *              SECTION: Time tracking
 * 2487:     private function timeTracking_init( )
 * 2525:     public function timeTracking_log( $debugTrailLevel, $prompt )
 * 2595:     public function timeTracking_prompt( $debugTrailLevel, $prompt )
 *
 *              SECTION: Template
 * 2641:     public function getTemplate( )
 *
 * TOTAL FUNCTIONS: 14
...
class tx_browser_pi1 extends tslib_pibase
{
  ...
  var $rows;
  // [Array] The rows after localisation but before consolidation
  // #59669, dwildt, 1+
  public $rowsLocalised;
  // [Array] Uids of all rows (after consolidation but before limitation). Is used by the record browser.
        ...
        $str_browserEngineIndicator = '
          <div>
            <span style="background:#00003A;color:white;font-size:.85em;">
              <a style="color:white;cursor:pointer;" title="You can disable this flag in the extension manager. See section Basic.">&nbsp;TYPO3 Browser Engine 5.x - alpha - inofficial&nbsp;</a>
            </span>
          </div>';
        if ( $this->b_drs_warn )
        {
          $prompt = 'Browser engine 5.x is enabled';
          t3lib_div::devLog( '[OK/SQL] ' . $prompt, $this->extKey, -1 );
        }
        break;
    }
...
//$this->dev_var_dump( $this->arr_extConf );
    ...
    // #i0084, 140922, dwildt, 1+
    $this->init_globalVars();
    $this->init_classVars();
    // Set the class variables
    //////////////////////////////////////////////////////////////////////
    //
...
//    $this->conf = $this->objZz->substitute_t3globals_recurs( $this->conf );
    ...
    if ( $this->objDownload->str_typeNum == 'download' )
    {
      ...
      return $this->objWrapper4x->wrapInBaseIdClass( $prompt_error );
    }
    // Download: send the file ...
    //////////////////////////////////////////////////////////////////////
    //
    ...
    if ( $arr_result[ 'error' ][ 'status' ] )
    {
      ...
      return $html_updateCheck . $this->objWrapper4x->wrapInBaseIdClass( $prompt );
    }
    // RETURN error
    ...
    {
      $prompt = $arr_result[ 'error' ][ 'header' ] . $arr_result[ 'error' ][ 'prompt' ];
      return $this->objWrapper4x->wrapInBaseIdClass( $prompt );
    }
    ...
    if ( $arr_result[ 'error' ][ 'status' ] )
    {
      ...
      return $html_updateCheck . $this->objWrapper4x->wrapInBaseIdClass( $template );
    }
    // RETURN error
    ...
    switch ( $this->view )
    {
      case( 'list' ):
        ...
        break;
      case( 'single' ):
        $str_template_completed = $this->objViewsingle->main();
        break;
    }
    // SWITCH view
    // Prompt the expired time to devlog
      ...
      case( 'map' ) :
        switch ( $this->objMap->enabled )
        {
          ...
          default :
            ...
            $header = 'FATAL ERROR!';
            $text = 'Unexpected value for the TypoScript property map.enabled: "' . $this->enabled . '"';
...
//$this->dev_var_dump( $str_template_completed );
   ...
   * @since   5.0.0
   */
  public function drs_die( $header, $text, $prompt = null, $level = 1 )
  {
    $strDebugTrail = t3lib_utility_Debug::debugTrail();
    $arrDebugTrail = explode( '//', $strDebugTrail );
    // Get class, method
...
#    var_dump(__METHOD__, __LINE__, $strDebugTrail);
  ...
  //public function dev_var_dump( )
  {
    ...
    $prompt = '<pre style="background:white;">' . $debugTrail[ 'prompt' ] . PHP_EOL .
            '</pre>' . PHP_EOL;
    echo $prompt;
    ...
    for ( $i = 0; $i < $numargs; $i++ )
    {
      ...
      $prompt = '<pre style="background:white;">type: ' . $type . PHP_EOL .
              $arg_list[ $i ] . PHP_EOL .
              '</pre>' . PHP_EOL;
      // Concatenate method, line, type and content. Wrap it with <pre>
      // Generate the prompt
  ...
  private function require_classes()
  {
    ...
    // Class with methods for the list view
    require_once( 'class.tx_browser_pi1_viewsingle.php' );
    $this->objViewsingle = new tx_browser_pi1_viewsingle( $this );

    // Class with wrapper methods for wrapping fields and link values (depreccated)
    require_once('class.tx_browser_pi1_wrapper4x.php');
    $this->objWrapper4x = new tx_browser_pi1_wrapper4x( $this );

//      // Class with wrapper methods for wrapping fields and link values (depreccated)
//    require_once('class.tx_browser_pi1_wrapper5x.php');
//    $this->objWrapper5x = new tx_browser_pi1_wrapper5x( $this );
  ...
  private function init_classVars()
  {
    ...
    $this->objFltr4x->conf_view = $this->conf_view;
    // [String] TypoScript path to the current view. I.e. views.single.1
    $this->objFltr4x->conf_path = $this->conf_path;
    ...
    // [Array] The TypoScript configuration array of the current view
    $this->objLocalise3x->conf_view = $this->conf_view;
    // [String] TypoScript path to the current view. I.e. views.single.1
    $this->objLocalise3x->conf_path = $this->conf_path;
    ...
    // [Array] The TypoScript configuration array of the current view
    $this->objLocalise->conf_view = $this->conf_view;
    // [String] TypoScript path to the current view. I.e. views.single.1
    $this->objLocalise->conf_path = $this->conf_path;
    ...
    // [Array] The TypoScript configuration array of the current view
    $this->objNavi_3x->conf_view = $this->conf_view;
    // [String] TypoScript path to the current view. I.e. views.single.1
    $this->objNavi_3x->conf_path = $this->conf_path;
    ...
    // [Array] The TypoScript configuration array of the current view
    $this->objNaviIndexBrowser->conf_view = $this->conf_view;
    // [String] TypoScript path to the current view. I.e. views.single.1
    $this->objNaviIndexBrowser->conf_path = $this->conf_path;
    ...
    // [Array] The TypoScript configuration array of the current view
    $this->objNaviModeSelector->conf_view = $this->conf_view;
    // [String] TypoScript path to the current view. I.e. views.single.1
    $this->objNaviModeSelector->conf_path = $this->conf_path;
    ...
    // [Array] The TypoScript configuration array of the current view
    $this->objNaviPageBrowser->conf_view = $this->conf_view;
    // [String] TypoScript path to the current view. I.e. views.single.1
    $this->objNaviPageBrowser->conf_path = $this->conf_path;
    ...
    // [Array] The TypoScript configuration array of the current view
    $this->objNaviRecordBrowser->conf_view = $this->conf_view;
    // [String] TypoScript path to the current view. I.e. views.single.1
    $this->objNaviRecordBrowser->conf_path = $this->conf_path;
    ...
    // [Array] The TypoScript configuration array of the current view
    $this->objSession->conf_view = $this->conf_view;
    // [String] TypoScript path to the current view. I.e. views.single.1
    $this->objSession->conf_path = $this->conf_path;
    ...
    // [Array] The TypoScript configuration array of the current view
    $this->objSocialmedia->conf_view = $this->conf_view;
    // [String] TypoScript path to the current view. I.e. views.single.1
    $this->objSocialmedia->conf_path = $this->conf_path;
    ...
    // [Array] The TypoScript configuration array of the current view
    $this->objSqlAut->conf_view = $this->conf_view;
    // [String] TypoScript path to the current view. I.e. views.single.1
    $this->objSqlAut->conf_path = $this->conf_path;
    ...
    // [Array] The TypoScript configuration array of the current view
    $this->objSqlFun->conf_view = $this->conf_view;
    // [String] TypoScript path to the current view. I.e. views.single.1
    $this->objSqlFun->conf_path = $this->conf_path;
    ...
    // [Array] The TypoScript configuration array of the current view
    $this->objSqlInit->conf_view = $this->conf_view;
    // [String] TypoScript path to the current view. I.e. views.single.1
    $this->objSqlInit->conf_path = $this->conf_path;
    ...
    // [Array] The TypoScript configuration array of the current view
    $this->objStat->conf_view = $this->conf_view;
    // [String] TypoScript path to the current view. I.e. views.single.1
    $this->objStat->conf_path = $this->conf_path;
    ...
    // [Array] The TypoScript configuration array of the current view
    $this->objTca->conf_view = $this->conf_view;
    // [String] TypoScript path to the current view. I.e. views.single.1
    $this->objTca->conf_path = $this->conf_path;
...
//    $this->objTemplate->conf      = $this->conf;
    ...
    // [Array] The TypoScript configuration array of the current view
    $this->objTemplate->conf_view = $this->conf_view;
    // [String] TypoScript path to the current view. I.e. views.single.1
    $this->objTemplate->conf_path = $this->conf_path;
    ...
    // [Array] The TypoScript configuration array of the current view
    $this->objTyposcript->conf_view = $this->conf_view;
    // [String] TypoScript path to the current view. I.e. views.single.1
    $this->objTyposcript->conf_path = $this->conf_path;
    ...
    // [Array] The TypoScript configuration array of the current view
    $this->objViewlist_3x->conf_view = $this->conf_view;
    // [String] TypoScript path to the current view. I.e. views.single.1
    $this->objViewlist_3x->conf_path = $this->conf_path;
    ...
    // [Array] The TypoScript configuration array of the current view
    $this->objViewlist->conf_view = $this->conf_view;
    // [String] TypoScript path to the current view. I.e. views.single.1
    $this->objViewlist->conf_path = $this->conf_path;
    // class.tx_browser_pi1_viewlist.php
    //////////////////////////////////////////////////////////////////////
    //
      // class.tx_browser_pi1_viewsingle.php
    // [Array] The current TypoScript configuration array
    $this->objViewsingle->conf = $this->conf;
    // [Integer] The current mode (from modeselector)
    $this->objViewsingle->mode = $this->piVar_mode;
    // [String] 'single' or 'single': The current view
    $this->objViewsingle->view = $this->view;
    // [Array] The TypoScript configuration array of the current view
    $this->objViewsingle->conf_view = $this->conf_view;
    // [String] TypoScript path to the current view. I.e. views.single.1
    $this->objViewsingle->conf_path = $this->conf_path;
    // class.tx_browser_pi1_viewsingle.php
  }
...
//      $this->b_drs_perform    = true;
    ...
    if ( $this->arr_extConf[ 'drs_mode' ] == 'ORDER BY' )
    {
      $this->b_drs_error = true;
      $this->b_drs_warn = true;
      //$this->b_drs_info       = true;
    ...
    if ( $this->arr_extConf[ 'drs_mode' ] == 'Typoscript' )
    {
      $this->b_drs_error = true;
      $this->b_drs_warn = true;
      $this->b_drs_info = true;
    ...
    if ( $this->arr_extConf[ 'drs_mode' ] == 'TYPO3 6.x' )
    {
      $this->b_drs_error = true;
      $this->b_drs_warn = true;
      $this->b_drs_info = true;
   ...
   * @since   6.0.0
   */
  private function init_globalVars()
  {
    $this->conf_view = $this->conf[ 'views.' ][ $this->view . '.' ][ $this->piVar_mode . '.' ];
    $this->conf_path = 'views.' . $this->view . '.' . $this->piVar_mode . '.';
  }
  ...
  /**
   * getTypo3Version( ): Get the current TYPO3 version
   *
   * @return	void
   * @internal #61520
   * @version 6.0.0
...
//    t3lib_div::devLog('[INFO/PERFORMANCE] ' . $prompt, $this->extKey, 0 );
   ...
   * @since   6.0.0
   */
  public function get_confView()
  {
    return $this->conf_view;
  }
  ...
  public function getTemplate()
  {
    ...
    // Error prompt: there isn't any template
