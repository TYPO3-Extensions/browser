...
class tx_browser_pi1_navi_pageBrowser
{
  ...
  public function get($content)
  {
    ...
    $pageBrowser = $this->pObj->pi_list_browseresults
            (
            $confPageBrowser['showResultCount'], $confPageBrowser['tableParams'], $confPageBrowser['wrap.'], $confPageBrowser['pointer'], $confPageBrowser['hscText']
    );
    // Get the wrapped pagebrowser
    ...
    if (empty($pageBrowser))
    {
      if ($this->pObj->b_drs_warn)
      {
        ...
        t3lib_div::devlog('[INFO/NAVI] ' . $prompt, $this->pObj->extKey, 2);
      }
    }
    else
    {
  ...
  private function requirementsRoute()
  {
    ...
    switch ($this->pObj->objMap->enabled)
    {
      case( 'Map +Routes' ) :
        if ($this->pObj->b_drs_warn)
        {
          $prompt = 'Sorry, pageBrowser isn\'t possible. Map +Routes is used.';
