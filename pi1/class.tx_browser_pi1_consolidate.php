 ...
 *
 *
 *   60: class tx_browser_pi1_consolidate
 *   81:     function __construct($parentObj)
 *   98:     public function consolidate($rows)
 *  560:     private function init_arrConsolidation()
 *  632:     public function addUidAndPid()
 *  709:     public function children_relation()
 *  825:     private function fields_wi_relation_bak()
 *  886:     private function fields_wi_relation()
 *  940:     private function fields_wi_marker($arr_fields_wi_relation)
 *  984:     private function setRowFields()
 * 1017:     private function tsConf_TEXT_path_wi_marker($arr_fields_wi_relation)
 * 1120:     private function manipulate_tsConf($arr_tsConf_TEXT_path_wi_marker, $arr_fields_wi_relation)
 *
 * TOTAL FUNCTIONS: 11
 ...
 * @since    3.4.4
 */
  public function consolidate($rows)
  {
...
//                if( ! $rows_cons[ $int_count ][ $tableField ] )
    ...
    //var_dump( __METHOD__, __LINE__, $rows_new );
    return $arr_return;
  }
 ...
 */
  private function init_arrConsolidation()
  {
    $conf = $this->pObj->conf;
    $mode = $this->pObj->piVar_mode;
    $view = $this->pObj->view;
 ...
 */
  public function children_relation()
  {
    ...
    // ABORTED  : rows contain more than one row
    if (count($rows) > 1)
    {
      if ($this->pObj->b_drs_warn)
      {
  ...
  /**
 * fields_wi_relation_bak():  Find every field with a relation to a foreign table.
 ...
 * @version   3.4.4
 */
  private function fields_wi_relation_bak()
  {
    $arr_fields_wi_relation = null;
    $tables = array();

    // Get all tables
    foreach ((array) $this->fields as $tableField)
    {
    ...
    foreach ($tables as $table)
    {
      ...
      {
        // CONTINUE : current tableField hasn't configured any foreign_table
        if (empty($GLOBALS['TCA'][$table]['columns'][$field]['config']['foreign_table']))
        {
          continue;
        }

        // current tableField has configured a foreign_table
        $tableField = $table . '.' . $field;
        $arr_fields_wi_relation[$tableField]['foreign_table'] = $GLOBALS['TCA'][$table]['columns'][$field]['config']['foreign_table'];
        $arr_fields_wi_relation[$tableField]['MM'] = $GLOBALS['TCA'][$table]['columns'][$field]['config']['MM'];
        if (empty($arr_fields_wi_relation[$tableField]['MM']))
        {
          unset($arr_fields_wi_relation[$tableField]['MM']);
        }
        $arr_fields_wi_relation[$tableField]['MM_opposite_field'] = $GLOBALS['TCA'][$table]['columns'][$field]['config']['MM_opposite_field'];
 ...
 * @version   3.4.4
 */
  private function fields_wi_relation()
  {
    $arr_fields_wi_relation = null;
 ...
 * @version   3.4.4
 */
  private function fields_wi_marker($arr_fields_wi_relation)
  {
    $conf = $this->pObj->conf;
    $mode = $this->pObj->piVar_mode;
    $view = $this->pObj->view;
  ...
  /**
 * setRowFields() : Set the globals $row and $fields
 *
 * @return	void
 * @internal  #59579
 * @version   5.0.0
 ...
 */
  private function setRowFields()
  {
    ...
    if (is_array($rows))
    {
      reset($rows);
      $firstKey = key($rows);
      $this->row = $rows[$firstKey];
      $this->fields = array_keys($rows[$firstKey]);
      return;
    }
 ...
 * @version   3.4.4
 */
  private function tsConf_TEXT_path_wi_marker($arr_fields_wi_relation)
  {
    $conf = $this->pObj->conf;
    $mode = $this->pObj->piVar_mode;
    $view = $this->pObj->view;
...
//var_dump(__METHOD__, __LINE__, $arr_fields_wi_relation);
 ...
 * @version   3.4.4
 */
  private function manipulate_tsConf($arr_tsConf_TEXT_path_wi_marker, $arr_fields_wi_relation)
  {
    $conf = $this->pObj->conf;
    $mode = $this->pObj->piVar_mode;
    $view = $this->pObj->view;
    ...
    foreach (array_keys((array) $arr_tsConf_TEXT_path_wi_marker) as $key_TEXT)
    {
      ...
      if ($bool_handle)
      {
        if (!empty($MM))
        {
          ...
          if ($this->pObj->b_drs_warn)
          {
            t3lib_div::devlog('[WARN/SQL] manipulate_tsConf(): Can\'t handle MM relation now!', $this->pObj->extKey, 2);
          }
        }
      }
      // WARN can't handle MM relation
      ...
      {
        // 121211, dwildt, 1-
        //$arr_uid_foreign = explode(',', $this->row[$tableField]);

        $select_fields = '*';
        $from_table = $foreign_table;
        $where_clause = 'uid IN (' . $this->row[$tableField] . ')';
        $orderBy = null;
