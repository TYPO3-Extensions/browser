 ...
 *  Copyright notice
 *
 *  (c) 2009-2014 - Dirk Wildt <http://wildt.at.die-netzmacher.de>
 *  All rights reserved
 *
...
/**
 * The class tx_browser_pi1_filter_3x bundles methods for rendering and processing filters and category menues
 *
 ...
 *
 *
 *   86: class tx_browser_pi1_filter_3x
 ...
 *
 */
class tx_browser_pi1_filter_3x
{
...
//              var_dump(__METHOD__ . ' (' . __LINE__ . '): in_array( ' . $strtolower_value . ', array( ' . $value_list . ' ) ) ' );
          ...
          default:
            $header = 'FATAL ERROR!';
            $text = 'key "' . key( $condition ) . '" is undefined.';
...
//          break;
        default:
          echo 'tx_browser_pi1_filter_3x::rednerHtmlFilter: undefined value in switch ' . $this->pObj->objCal->arr_area[ $tableField ][ 'key' ];
          exit;
      }
      // DRS - Development Reporting System
...
//        $this->pObj->elements = $this->pObj->elements + $markerArray;
  ...
  private function get_treeRendered( $arr_ts )
  {
    ...
    $endTag = '</li>' .
            str_repeat
                    (
                    '</ul>' . PHP_EOL . $indent . '</li>', $curr_depth
            ) .
            PHP_EOL .
            $indent . '</ul>' . PHP_EOL;
...
//    }
   ...
   * @since 3.6.0
   */
  public function get_wrappedItemSelected( $uid, $value, $arr_piVar, $arr_ts, $conf_selected, $conf_item )
  {
    //$pos = strpos($this->pObj->str_developer_csvIp, t3lib_div :: getIndpEnv('REMOTE_ADDR'));
    //if ( ! ( $pos === false ) )
    //{
...
if ( defined( 'TYPO3_MODE' ) && $TYPO3_CONF_VARS[ TYPO3_MODE ][ 'XCLASS' ][ 'ext/browser/pi1/class.tx_browser_pi1_filter_3x.php' ] )
{
  include_once ($TYPO3_CONF_VARS[ TYPO3_MODE ][ 'XCLASS' ][ 'ext/browser/pi1/class.tx_browser_pi1_filter_3x.php' ]);
}
?>