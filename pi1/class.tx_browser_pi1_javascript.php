...
//if (!($pos === false)) var_dump('jss 138', $conf_object);
    ...
    {
      t3lib_div::devlog( '[INFO/JSS] AJAX onchange marker is ###' . strtoupper( $class_onchange ) . '###', $this->pObj->extKey, 0 );
      t3lib_div::devlog( '[INFO/JSS] AJAX onchange class is ' . $class_onchange, $this->pObj->extKey, 0 );
      t3lib_div::devlog( '[HELP/JSS] Change it? Configure javascript.ajax.html.marker.ajax_onchange', $this->pObj->extKey, 1 );
    }
    // DRS - Develoment Reporting System
    // Get the class name and marker
    //////////////////////////////////////////////////////////////////////
    ...
    if ( !$arr_ts[ 'ajax_onchange' ] )
    {
      ...
      if ( $this->pObj->b_drs_javascript )
      {
        t3lib_div::devlog( '[INFO/JSS] ' . $obj_ts . ' shoudn\'t get an AJAX onload class.', $this->pObj->extKey, 0 );
        t3lib_div::devlog( '[HELP/JSS] Change it? Configure ajax_onchange for ' . $obj_ts . '.', $this->pObj->extKey, 1 );
      }
      $bool_return = true;
    }
...
//if (!($pos === false)) var_dump('jss 215', $arr_ts['wrap.']['item.']['class']);
    ...
    switch ( $obj_ts )
    {
      ...
      case('CHECKBOX'):
        ...
        if ( $number_of_items >= 2 )
        {
          ...
          if ( $this->pObj->b_drs_javascript )
          {
            t3lib_div::devlog( '[INFO/JSS] ' . $obj_ts . ': Number of items are more than one: onchange class becomes null.', $this->pObj->extKey, 0 );
          }
          // DRS - Develoment Reporting System
        }
        // Don't reload by AJAX, if there is more than one checkbox
      ...
      case('SELECTBOX'):
        ...
        if ( $number_of_items >= 2 )
        {
          ...
          if ( $arr_ts[ 'multiple' ] == 1 && $arr_ts[ 'size' ] >= 2 )
          {
            ...
            if ( $this->pObj->b_drs_javascript )
            {
              t3lib_div::devlog( '[INFO/JSS] Number of items are more than one, multiple is true and size is bigger than one: onchange class becomes null.', $this->pObj->extKey, 0 );
            }
            // DRS - Develoment Reporting System
            $conf_object = str_replace( $marker_onchange, null, $conf_object );
          }
    ...
    if ( $this->pObj->b_drs_javascript )
    {
      t3lib_div::devlog( '[INFO/JSS] AJAX onload class is: ' . $class_onchange, $this->pObj->extKey, 0 );
    }
    // DRS - Develoment Reporting System
    // Remove empty class
    $conf_object = str_replace( ' class=""', null, $conf_object );
...
//      return true;
    ...
    if( $footer)
    {
      $bool_success = $this->load_jQueryFooter( $path, $name, $path_tsConf );
    }
    else
    {
      $bool_success = $this->load_jQueryHeader( $path, $name, $path_tsConf );
    }
  ...
  /**
   * load_jQueryHeader(): Add a JavaScript file at the header of the page (the footer section)
   *
   ...
   * @since       6.0.2
   */
  private function load_jQueryHeader( $path, $name, $keyPathTs )
  {
    $key = $this->pObj->extKey . '_' . $name;
    if ( isset( $GLOBALS['TSFE']->pSetup['includeJS.'][ $key ] ) )
    {
      return true;
    }

    $GLOBALS['TSFE']->pSetup['includeJS.'][ $key ] = $path;
    $GLOBALS['TSFE']->pSetup['includeJS.'][ $key . '.' ][ 'forceOnTop' ] = 1;
    ...
    $prompt = 'page.includeJS.' . $key . ' = ' . $path;
    t3lib_div::devlog( '[INFO/FLEXFORM+JSS] ' . $prompt, $this->pObj->extKey, 0 );
  ...
  /**
   * load_jQueryFooter(): Add a JavaScript file at the bottom of the page (the footer section)
   *
   * @param	string		$path         : relative path to the Javascript
   * @param	string		$name         : For the key of additionalHeaderData
   * @param	string		$keyPathTs    : The TypoScript element path to $path for the DRS
   ...
   * @since       6.0.2
   */
  private function load_jQueryFooter( $path, $name, $keyPathTs )
  {
    $key = $this->pObj->extKey . '_' . $name;
    if ( isset( $GLOBALS['TSFE']->pSetup['includeJSFooter.'][ $key ] ) )
    {
      return true;
    }

    $GLOBALS['TSFE']->pSetup['includeJSFooter.'][ $key ] = $path;
    $GLOBALS['TSFE']->pSetup['includeJSFooter.'][ $key . '.' ][ 'forceOnTop' ] = 1;
    ...
    $prompt = 'page.includeJSFooter.' . $key . ' = ' . $path;
    t3lib_div::devlog( '[INFO/FLEXFORM+JSS] ' . $prompt, $this->pObj->extKey, 0 );
    $prompt = 'Change the path? Configure: \'' . $keyPathTs . '\'';
    t3lib_div::devlog( '[HELP/FLEXFORM+JSS] ' . $prompt, $this->pObj->extKey, 1 );
    $prompt = 'Change the section for all JSS files? Take the Constant Editor: [Browser - JAVASCRIPT]';
...
' . implode( '', file( $absPath ) ) . '
        ...
        if ( !$inline )
        {
          $GLOBALS[ 'TSFE' ]->additionalHeaderData[ $this->pObj->extKey . '_' . $name ] = '  <link rel="stylesheet" type="text/css" href="' . $path . '" media="all" />';
        }
        break;
      // #50069, 130716, dwildt, -
//      case( 'jss' ):
