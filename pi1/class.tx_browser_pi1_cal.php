...
//}
    ...
    for
    (
    $sec_time_unit_curr = $sec_begin_abs; $sec_time_unit_curr < $sec_end_abs; $sec_time_unit_curr = $sec_time_unit_curr + $sec_time_unit
    )
    {
...
//$this->pObj->dev_var_dump( $arr_periods );
  ...
  private function cal_group_check()
  {
    ...
    if ( $this->pObj->b_drs_cal )
    {
      t3lib_div :: devLog( '[OK/CAL/UI] Group is configured. HTML template contains subparts CAL_DATE_GROUP.', $this->pObj->extKey, -1 );
    }
...
//    }
    ...
    for ( $int_element = 10; $int_element <= ($times * 10); $int_element = $int_element + 10 )
    {
      ...
      if ( $this->pObj->b_drs_error )
      {
        ...
        if ( rawurlencode( $fromValue ) != $fromUrl )
        {
          $prompt = $tableField . ': value_stdWrap (' . rawurlencode( $fromValue ) . ') and url_stdWrap (' . $fromUrl . ') hasn\'t the same configuration. Filter won\'t run proper!';
