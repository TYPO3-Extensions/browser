...
//var_dump( __METHOD__, __LINE__, $arr_return, $this->pObj->arr_resultphrase);
    ...
    // #60013, 140701, dwildt, 4+
    // Delete piVar[sort], if it is empty.
    if ( empty( $this->pObj->piVars[ 'sort' ] ) )
    {
...
//}
   ...
   * @return	array		$arr_multi_dimensional: The current Multi-dimensional array with substituted markers
   */
  public function extend_marker_wi_pivars( $markerArray )
  {
  ...
  function linkTP( $str, $typolink = array(), $urlParameters = array(), $cache = 0, $altPageId = 0 )
  {
    ...
    $typolink[ 'additionalParams' ] = $typolink[ 'parent.' ][ 'addParams' ] . t3lib_div::implodeArrayForUrl( '', $urlParameters, '', 1 ) . $this->pObj->pi_moreParams;
  ...
  /**
   * get_advanced_5_0_0_useTyposcriptEngine4x()
   *
   * @return	boolean		$cHash_md5: md5 value like d218cfedf9
   *
   * @version   5.0.0
   * @since     5.0.0
   */
  public function get_advanced_5_0_0_useTyposcriptEngine4x()
  {
    static $bool_advanced_5_0_0_useWrapper4x = null;

    if ( $bool_advanced_5_0_0_useWrapper4x !== null )
    {
      return $bool_advanced_5_0_0_useWrapper4x;
    }

    $conf = $this->pObj->conf;
    $view = $this->pObj->view;
    $mode = $this->pObj->mode;
    $conf_view = $conf[ 'views.' ][ $view . '.' ][ $mode . '.' ];

    if ( !empty( $conf_view[ 'advanced.' ] ) )
    {
      $bool_advanced_5_0_0_useWrapper4x = $conf_view[ 'advanced.' ][ 'downgrade.' ][ '5_0_0.' ][ 'typoscriptEngine4x' ];
      return $bool_advanced_5_0_0_useWrapper4x;
    }

    $bool_advanced_5_0_0_useWrapper4x = $conf[ 'advanced.' ][ 'downgrade.' ][ '5_0_0.' ][ 'typoscriptEngine4x' ];
    return $bool_advanced_5_0_0_useWrapper4x;
  }
...
//        $cHash_array  = t3lib_cacheHash( $str_params );
   ...
   * @version   4.8.5
   */
  public function children_tsconf_recurs( $key, $arr_multi_dimensional, $str_devider )
  {
    static $int_levelRecurs = 0;
    $int_levelRecurs++;

    // Security: recursionGuard
    $this->children_tsconf_recursDie( $int_levelRecurs );
  ...
  /**
   * children_tsconf_recursDie: Die if recursion level is unproper
   *
   * @param	integer		$int_levelRecurs: current recursion level
   * @return	void
   *
   * @version   5.0.0
   */
  private function children_tsconf_recursDie( $int_levelRecurs )
  {
    $int_levelRecursMax = $this->int_advanced_recursionGard;

    // RETURN : recursion level is proper
    if ( $int_levelRecurs <= $int_levelRecursMax )
    {
      return;
    }
    ...
    // DIE  : recursion level is unproper
    $header = null;
    $text = null;
  ...
  {
//    require_once('class.tx_browser_pi1_zz_sword.php');
//    $this->objZzSword = new tx_browser_pi1_zz_sword( $this );
//    $arr_return = $this->objZzSword->main( );
//    return $arr_return;
    ...
    // #60107, dwildt, 1-
    //$str_sword_phrase = $this->pObj->piVar_sword;
    // #60107, dwildt, 1+
    $str_sword_phrase = trim( $this->pObj->piVar_sword );
    if ( !$str_sword_phrase )
    {
      // DRS - Development Reporting System
    ...
    $arr_sql_operator[ 'and' ] = $this->pObj->objWrapper4x->general_stdWrap( $conf_and[ 'value' ], $conf_and );
    ...
    $arr_sql_operator[ 'or' ] = $this->pObj->objWrapper4x->general_stdWrap( $conf_or[ 'value' ], $conf_or );
    ...
    $arr_sql_operator[ 'not' ] = $this->pObj->objWrapper4x->general_stdWrap( $conf_not[ 'value' ], $conf_not );
    ...
    // 140705, dwildt, 1+
    $arr_swords_quoted = explode( '"', $str_sword_phrase );
//var_dump( __METHOD__, __LINE__, $str_sword_phrase);
    // Preparation for investigating quotes
    //////////////////////////////////////////////////
    //
    ...
    if ( count( $arr_swords_quoted ) > 1 )
    {
      ...
      // Loop through the array with the quoted and non quoted swords
      $int_counter = 0;
...
//var_dump(__METHOD__, __LINE__, $arr_swords_exploded);
...
//    $arr_replace = array( 'or', 'or' );
    // #140716, dwildt, 2+
    $arr_search = array( $arr_sql_operator[ 'or' ], ' ' );
    $arr_replace = array( 'or', 'and' );
    $str_sword_mask = str_replace( $arr_search, $arr_replace, $str_sword_mask );
//var_dump( __METHOD__, __LINE__, $arr_search, $arr_replace, $str_sword_mask );
...
//var_dump( __METHOD__, __LINE__, $str_sword_mask, $str_resultphrase_mask );
...
//var_dump( __METHOD__, __LINE__, $arr_swords );
    // Get all NOT
    // Get all OR
    $int_counter = 0;
    $arr_or = explode( 'or', $str_sword_mask_wo_not );
//var_dump( __METHOD__, __LINE__, $str_sword_mask_wo_not, $arr_or );
...
//var_dump( __METHOD__, __LINE__, $arr_or, $arr_swords );
    // Get all OR
...
//var_dump( __METHOD__, __LINE__, $arr_search_or );
...
//var_dump(__METHOD__, __LINE__, $str_proper_search);
