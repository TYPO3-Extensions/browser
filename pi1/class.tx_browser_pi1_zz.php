 ...
 *  141:     function __construct($parentObj)
 *
 *              SECTION: piVars
 *  174:     function prepairePiVars()
 *  759:     function removeFiltersFromPiVars($inputPiVars, $filterConf)
 *  815:     function advanced_remove_piVars($keepFilters=0)
 *  921:     function advanced_remove_piVars_filter()
 *
 *              SECTION: $GLOBAL markers
 * 1049:     function get_t3globals_value($marker)
 * 1124:     function substitute_t3globals_recurs($arr_multi_dimensional)
 *
 *              SECTION: Session markers
 * 1302:     function session_marker($arr_tsConf, $elements)
 *
 *              SECTION: CSV process and format time
 * 1387:     function getCSVasArray($csvValues)
 * 1403:     function getCSVtablefieldsAsArray($csvTableFields)
 * 1422:     function cleanUp_lfCr_doubleSpace($csvValue)
 * 1447:     function setTsStrftime()
 *
 *              SECTION: Link
 * 1497:     function linkTP($str, $typolink=array(), $urlParameters=array(), $cache=0, $altPageId=0)
 * 1532:     function linkTP_keepPIvars($str, $typolink=array(), $overrulePIvars=array(), $cache=0, $clearAnyway=0, $altPageId=0)
 * 1566:     function get_absUrl($str_relUrl)
 * 1601:     function get_singlePid_for_listview()
 * 1661:     function get_cHash($str_params)
 * 1684:     function get_pathWoEXT($str_TYPO3_EXT_path)
 *
 *              SECTION: Markers
 * 1736:     function substitute_marker_recurs($arr_multi_dimensional, $elements)
 * 2029:     function extend_marker_wi_pivars($markerArray)
 *
 *              SECTION: TypoScript children records
 * 2074:     function children_tsconf_recurs($key, $arr_multi_dimensional, $str_devider)
 *
 *              SECTION: Languages, _LOCAL_LANG
 * 2166:     function getTableFieldLL($tableField)
 * 2258:     function initLang()
 *
 *              SECTION: Sword and Search respectively
 * 2302:     function search_values($str_sword_phrase)
 * 2746:     function color_swords($tableField, $value)
 *
 *              SECTION: Security
 * 2839:     function secure_piVar($str_value, $str_type)
 *
 *              SECTION: TCA
 * 3005:     function loadTCA($str_table)
 *
 *              SECTION: UTF-8
 * 3051:     function b_TYPO3_utf8()
 * 3129:     function char_single_multi_byte($str_char)
 *
 * TOTAL FUNCTIONS: 29
