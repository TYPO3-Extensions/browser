...
window.Tooltip = function( data ){
    ...
    self.show = function( event, element, icon, mouse ){
    ...
    self.setup = (function(){
        if( !event ){
            new Error( 'event', null, oxMap.cfg.error );
            event = 'click';
        }

        eventHandler = oxMap.Utils.getEvent( event ).split(/\s/);
        mainEvent = eventHandler[ 0 ];

        if( mainEvent === 'click' ){
            hrefNoFollow = oxMap.OSM.map.div.getElementsByTagName('a');
            for( var a = 0, b = hrefNoFollow.length; a < b; a += 1 ){
            	var attrClass = hrefNoFollow[ a ].getAttribute('class');
            	if( attrClass ){
            		if( attrClass.match(/oxMap-markerLink/) ){
