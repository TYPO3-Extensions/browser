...
var oxMapRender = function( )
{
	...
	this.init = function( oConfig )       //	default initialize method
  {
		if( ! oConfig )                     //	needs map configuration like oxMapConfig.js
    {
		...
		var oWmsLayer   = null;
		var oWmsConfig  = { };
		var impr        = ( function( )     //	define imprint and license with WMS -> have to set
      {
		...
		var mapOptions = {									//	define map options from configuration
			controls:       (
                        function( )     //	control types from oxMapConfig.MapControls
                        {
    ...
    if( self.oxMapConfig[aAllowedWMS[a]] )
    {
      ...
      break;
		...
		oxMap.setCenter( oCenterCoor, oWmsConfig.zoom );
...
//		}, 25 );
	...
	this.setMap = function( oxMapId, oxMapOptions ) //	control types from oxMapConfig.MapControls
  {
		return new OpenLayers.Map( oxMapId, oxMapOptions );
	};
	
	this.addDefaultMapLayer = function( sWms, sWmsConfig )
  {                                               //	create default map layer, default OSM.Mapnik
		if( sWms == 'OSM' )
    {
			switch ( sWmsConfig )
      {
	...
	this.setCoordinates = function( aCoorPair )
  {                                               //	return OpenLayers.LonLat object
	...
	this.setMarkerWithLayer = function( oMarker )
  {													//	set marker with has an infobox
		var box;
		var sCategory = null;
		var oCategoryData = null;

		for( sCategory in oMarker )
    {																	//	check category for data
  ...
  {																//	set standalone marker with further meta data
    ...
    {																	//	check category for data
			...
			for( var a = 0, b = oCategoryData.data.length; a < b; a++ )
      {                                                           //	check coordinates
  ...
  {                                                               //	customized icon creater by using OpenLayers.Icon object
		var icon    = param[0];
	...
	this.filterLayer = function( layerName, switchTo )
  {												//	filter layer for drawing on map --> layerName or category, filter property on | off or value between 0 and 1
	...
	this.infoBox = function( oMap,sCategory, oCategoryData )
  {											//	build an infobox object form given data src .txt-file --> text.txt
...
if( oxMapConfig.MapMarker )
{                                                                 //	marker method should be started outside oxMapRender
